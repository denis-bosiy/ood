...
export class CModel implements IModel {
    ...
    public removeObserver(observer: IObserver): void {
        let indexOfRemovedObserver = this.m_observers.indexOf(observer);

        while (indexOfRemovedObserver !== -1) {
            this.m_observers.splice(indexOfRemovedObserver, 1);
            indexOfRemovedObserver = this.m_observers.indexOf(observer);
        }
    }

    public notifyObservers(): void {
        this.m_observers.forEach((observer: IObserver) => {
            observer.update();
        });
    }
}