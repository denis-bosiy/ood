...
import { CResizeShapeCommand } from "./CResizeShapeCommand";
...
describe("test CResizeShapeCommand", () => {
  beforeEach(() => {
    mockChangeShapeFrame.mockClear();
  });

  test("Executing should invoke changeShapeFrame with oldFrame.width + delta.x, oldFrame.height + delta.y ...", () => {
    ...
    const moveShapeCommand: ICommand = new CResizeShapeCommand(new CMockModel(), shape, delta);
    ...
    expect(mockChangeShapeFrame.mock.calls[0][1].x).toBe(frame.x);
    expect(mockChangeShapeFrame.mock.calls[0][1].y).toBe(frame.y);
    expect(mockChangeShapeFrame.mock.calls[0][1].width).toBe(frame.width + delta.x);
    expect(mockChangeShapeFrame.mock.calls[0][1].height).toBe(frame.height + delta.y);
  });

  test("Unexecuting should invoke changeShapeFrame with oldFrame.width + delta.x, oldFrame.height + delta.y ...", () => {
    const frame: CFrame = new CFrame(0, 0, 20, 20);
    const leftTopPoint: CPoint = new CPoint(20, 30);
    const shape: IShape = new CRectangle(frame, leftTopPoint, 20, 30);
    const delta: CVector = new CVector(3, 3);
