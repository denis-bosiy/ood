...
import { CSoldState } from "./CSoldState";
...
describe("test SoldState", () => {
    ...
    test("using refill method should invoke console.log and should not execute setGumsCount method of gumball machine", () => {
        const gumballMachine: IGumballMachine = new MockGumballMachine();
        const soldState: IState = new CSoldState(gumballMachine);

        soldState.refill(2);

        expect(console.log).toBeCalledTimes(1);
        expect((console.log as jest.Mock).mock.calls[0][0]).toBe("Can not change gums count while delivering a gumball");
        expect(mockSetGumsCount).toBeCalledTimes(0);
    });
});